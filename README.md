...
- [Материалы](#материалы)
    ...
    - [Ошибки сборки и решения](#ошибки-сборки-и-решения)
...
Отлично, изменение пакета попало в репозиторий. Теперь, чтобы при установке зависимостей применились наши изменения, остается запустить команду `postinstall`. 
...
### Ошибки сборки и решения

1. Ошибка:

    ERROR in js/app.bundle.js from UglifyJs  
    Unexpected token: keyword «const» [js/app.bundle.js:1654,0]
...
Валидацию W3C можно проводить на этом ресурсе: https://validator.w3.org/.
При валидации сайта не должно возникать ни ошибок ни предупреждений.
Самые сложные ошибки и способы их решения выписаны ниже.
...
1. Section lacks heading. Consider using h2-h6.
    ...
    Каждая секция сайта с тегом `<section>` должна иметь заголовок от `<h2>` до `<h6>`.
    
    Также по SEO заголовки должны размещаться следуя иерархии. То есть сначала должен идти h2, затем уже остальные h3-h6. Поэтому минимальным заголовком для секции будет h2.
    
    Если дизайном/версткой не предусмотрен заголовок, то добавляем скрытый заголовок с классом 'visually-hidden'.
    
    Пример: `<h2 class="visually-hidden">Блог</h2>`
...
2. Element source is missing required attribute srcset.
    ...
    Ошибка из-за поддержки webp. Тут нужно дублировать аттрибут src на data-src для изображений по такой структуре:

    ```html
    <img src="/img/base/hero.png" 
        data-src="/img/base/hero.png" 
        alt="">
    ```

    С подддержкой .webp прямо на верстке:

    ```html
    <picture>
        <source srcset="/img/base/hero.webp" 
            type="image/webp">
        <img src="/img/base/hero.png"
            data-src="/img/base/hero.png" 
            alt="">
...
1. Расширение Adblock блокирует изображения и секции по следующим критериям:

    - Название изображения содержит рекламные ключевые слова: 'ad', 'adv', 'banner', и.т.д.
    - Название изображения содержит размеры, например, 'img_300x250.jpg'
    - Изображение лежит в папке 'ad', 'adv', 'banner', 'banners'
    - Секция имеет класс/id, которые содержат 'ad', 'adv'
 
    Даже если у вас не включен Adblock, он может быть включен у пользователей, поэтому таких названий изображений, классов и путей лечше избегать.
